...
export async function listAvailableBackups(options = {}) {
   ...
   * 1. Fetches the file from IPFS gateway with timeout protection
   * 2. Validates Content-Type header (should be JSON/text)
   * 3. Checks file size (metadata files are small, < 100KB)
   * 4. Validates JSON structure (starts with '{' or '[')
   * 5. Detects binary data (non-printable characters)
   * 6. Parses as JSON and validates backup metadata structure
   * 7. Filters by spaceName if specified in metadata
   * 
  ...
  const checkFile = async (file) => {
    try {
      ...
      // Try to fetch with increased timeout - metadata files are small but may need time to propagate
      ...
      // Check Content-Type if available (helps identify JSON vs binary)
      const contentType = response.headers.get('content-type');
      if (contentType && !contentType.includes('json') && !contentType.includes('text')) {
        // Likely a binary file (CAR file), skip it
        logger.debug(`Skipping non-JSON file ${cid.substring(0, 12)}... (Content-Type: ${contentType})`);
        return null;
      }
      ...
      if (!trimmedText.startsWith('{') && !trimmedText.startsWith('[')) {
        ...
        logger.debug(`Skipping non-JSON file ${cid.substring(0, 12)}... (doesn't start with '{' or '[')`);
        return null;
      }
      ...
      if (/[\x00-\x08\x0E-\x1F]/.test(firstBytes)) {
        ...
        logger.debug(`Skipping binary file ${cid.substring(0, 12)}... (contains binary data)`);
        return null;
      }
      ...
      try {
        ...
        logger.debug(`Skipping invalid JSON file ${cid.substring(0, 12)}...: ${parseError.message}`);
        return null;
      }
      ...
      logger.debug(`Failed to check file ${file.root?.toString() || file.cid?.toString() || 'unknown'}: ${error.message}`);
      return null;
    }
  };
  ...
  while (attempt < maxRetries) {
    ...
    if (attempt > 1) {
      logger.info(`Retry attempt ${attempt}/${maxRetries} (waiting ${retryDelay}ms before retry)...`);
    ...
    logger.info(`Found ${backups.length} backups (attempt ${attempt}/${maxRetries})`);
    ...
    if (attempt === maxRetries) {
      logger.warn(`No backups found after ${maxRetries} attempts. Files may not be available on IPFS gateway yet.`);
    }
  }
...
export async function restoreFromSpaceCAR(orbitdb, options = {}) {
  ...
  try {
    ...
    if (!carResponse.ok) {
      throw new Error(
        `Failed to download CAR file after ${maxAttempts} attempts: ${carResponse.statusText}`,
      );
    }
