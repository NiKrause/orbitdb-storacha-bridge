  ...
  const checkFile = async (file) => {
    try {
      ...
      if (
        ...
        logger.debug(
          `Skipping non-JSON file ${cid.substring(0, 12)}... (Content-Type: ${contentType})`,
        );
        return null;
      }
  ...
  try {
    ...
    if (config.useIPFSNetwork && orbitdb?.ipfs) {
      try {
        ...
        // Get unixfs from orbitdb if available
        const { unixfs: unixfsModule } = await import("@helia/unixfs");
        const fs = unixfsModule(orbitdb.ipfs);
    ...
    if (!carBytes) {
      ...
      while (attempts < maxAttempts) {
        ...
        throw new Error(
          `Failed to download CAR file: ${carResponse.statusText}`,
        );
      }
