          ...
          try {
            ...
            if (carResponse.ok) {
              ...
              if (
                ...
                const tempBytes = new Uint8Array(await carResponse.arrayBuffer());
                const errorPageText = new TextDecoder("utf-8", { fatal: false }).decode(tempBytes);
                logger.warn(`   ⚠️ Gateway ${gateway} returned HTML error page (Content-Type: ${contentType})`);
                logger.debug(`   Error page preview: ${errorPageText.substring(0, 200)}...`);
              ...
              const textStart = new TextDecoder("utf-8", { fatal: false }).decode(
                carBytes.slice(0, Math.min(100, carBytes.length)),
              );
              if (
                textStart.trim().startsWith("<!DOCTYPE") ||
                textStart.trim().startsWith("<html") ||
                ...
                // Decode and log the error page
                const errorPageText = new TextDecoder("utf-8", { fatal: false }).decode(carBytes);
                logger.warn(`   ⚠️ Gateway ${gateway} returned HTML error page (detected by content)`);
                logger.debug(`   Error page preview: ${errorPageText.substring(0, 200)}...`);
              ...
              if (carBytes.length < 11) {
                logger.warn(`   ⚠️ Gateway ${gateway} returned file too small (${carBytes.length} bytes), trying next gateway...`);
                carBytes = null; // Reset to try next gateway
                break; // Try next gateway
              }
    ...
    if (manifestCID) {
      ...
      if (manifestInBlocks) {
        ...
        try {
          ...
          logger.warn(`   ⚠️ Could not verify manifest in blockstore: ${error.message}`);
        }
      }
    }
