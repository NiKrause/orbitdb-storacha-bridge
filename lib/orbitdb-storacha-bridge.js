 ...
 * @param {Object} [serviceConf] - Optional service configuration for custom URLs
 * @param {string} [serviceConf.access] - Custom access service URL
 * @param {string} [serviceConf.upload] - Custom upload service URL
 * @returns {Promise<Object>} - Initialized Storacha client
 */
async function initializeStorachaClient(storachaKey, storachaProof, serviceConf = undefined) {
  // Handle both multibase-encoded keys (production) and JSON archive strings (testing)
  let principal;
  try {
    // Try parsing as multibase string (production format)
    principal = Signer.parse(storachaKey);
  } catch (e) {
    // If that fails, try parsing as JSON archive (test format)
    try {
      const archive = JSON.parse(storachaKey);
      // Reconstruct Uint8Array from plain object (JSON serialization converts it)
      const fixedArchive = {
        id: archive.id,
        keys: {
          [archive.id]: new Uint8Array(Object.values(archive.keys[archive.id])),
        },
      };
      principal = Signer.from(fixedArchive);
    } catch (jsonError) {
      throw new Error(`Unable to parse storacha key: ${e.message}. Also failed as JSON: ${jsonError.message}`);
    }
  }
  ...
  // Add serviceConf if provided (for in-memory testing)
  // ServiceConf must be connection objects, not just URLs
  if (serviceConf) {
    
    // If serviceConf contains URLs, convert them to proper connections
    if (serviceConf.access instanceof URL || serviceConf.upload instanceof URL) {
      const { connect } = await import('@ucanto/client');
      const { CAR, HTTP } = await import('@ucanto/transport');
      const DID = await import('@ipld/dag-ucan/did');
      
      // Use a generic DID for test services
      const testServicePrincipal = DID.parse('did:web:test.storacha.local');
      
      clientOptions.serviceConf = {
        access: connect({
          id: testServicePrincipal,
          codec: CAR.outbound,
          channel: HTTP.open({
            url: serviceConf.access,
            method: 'POST',
          }),
        }),
        upload: connect({
          ...
          codec: CAR.outbound,
          channel: HTTP.open({
            url: serviceConf.upload,
            method: 'POST',
          }),
        }),
      };
    } else {
      // Already proper connections
...
async function uploadBlocksToStoracha(
  ...
  const uploadSingleBlock = async ([hash, blockData]) => {
    try {
      ...
      // If blockCache is provided, store there (for in-memory testing)
      if (blockCache) {
        logger.info(
          `   💾 Storing block ${hash} (${blockData.bytes.length} bytes) in blockCache...`,
        );
        blockCache.set(hash, blockData.bytes);
        uploadedCID = hash; // Use original CID as uploaded CID for in-memory mode
        logger.info(`   ✅ Stored in blockCache: ${hash}`);
      } else {
        // Normal Storacha upload
        ...
        logger.debug('   Client agent DID:', client.agent?.did?.());
        logger.debug('   Current space:', client.currentSpace?.()?.did());
...
export async function listStorachaSpaceFiles(options = {}) {
  ...
  try {
    ...
    if (options.ucanClient) {
      ...
      client = await initializeStorachaClient(storachaKey, storachaProof, options.serviceConf);
    }
...
async function downloadAndBridgeBlocks(
  ...
  for (const [originalCID, storachaCID] of cidMappings) {
    try {
      logger.info(`   📥 Processing ${storachaCID}...`);

      let blockBytes = null;
      
      // Try blockCache first (for in-memory testing)
      if (config.blockCache && config.blockCache.has(storachaCID)) {
        blockBytes = config.blockCache.get(storachaCID);
        logger.info(
          `   ✅ Retrieved ${blockBytes.length} bytes from blockCache`,
        );
      }

      // Try network download if blockCache didn't have it, and if enabled with Helia instance available
      if (!blockBytes && config.useIPFSNetwork && config.helia) {
        try {
...
export async function backupDatabase(orbitdb, databaseAddress, options = {}) {
  ...
  try {
    ...
    // Skip client initialization if blockCache is provided (in-memory testing)
    let client;
    ...
    if (!config.blockCache) {
      ...
      if (config.ucanClient) {
        ...
        client = await initializeStorachaClient(storachaKey, storachaProof, config.serviceConf);
      }
    } else {
      logger.info("💾 Using blockCache for in-memory testing (no client needed)...");
      client = null; // No client needed for blockCache mode
    }
...
export async function restoreDatabase(
  ...
  try {
    ...
    const { successful, matches } = await downloadAndBridgeBlocks(
      ...
      client,
      orbitdb.ipfs.blockstore,
      { ...config, helia: orbitdb.ipfs, blockCache: config.blockCache },
    );
