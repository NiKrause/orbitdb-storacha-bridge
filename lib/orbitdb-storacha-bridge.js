...
import { createStorachaBackend } from "./backends/storacha.js";
...
/**
 * Decide whether a backup goes up as individual blocks or as one CAR.
 *
 * Per-block upload is the older path and the more useful one when it works: every block
 * is separately addressable on the network, so a peer can fetch exactly what it misses.
 * It only works on a backend that stores the bytes we hand it under the CID we computed,
 * and that accepts objects as small as an OrbitDB entry. Filecoin Onchain Cloud does
 * neither -- 127-byte minimum, piece-addressed -- and it will not be the last one.
 *
 * So the default is not a guess about which is better; it is a question asked of the
 * backend. Callers who know what they want can still say so.
 *
 * @param {import("./backends/types.js").StorageBackend} backend
 * @param {"auto"|"blocks"|"car"} [requested="auto"]
 * @returns {"blocks"|"car"}
 */
export function chooseBackupStrategy(backend, requested = "auto") {
  const capabilities = backend?.capabilities || {};
  const canUploadBlocks =
    capabilities.preservesInnerCids === true && !capabilities.minBlobSize;
  ...
  if (requested === "blocks") {
    if (!canUploadBlocks) {
      throw new Error(
        `Backend "${backend?.name}" cannot store OrbitDB blocks individually ` +
          `(preservesInnerCids=${capabilities.preservesInnerCids}, minBlobSize=${capabilities.minBlobSize || 0}). ` +
          'Use strategy: "car".',
      );
    }
    return "blocks";
  ...
  return canUploadBlocks ? "blocks" : "car";
}
...
async function uploadBlocks(
  ...
  chooseBackupStrategy(backend, "blocks");
  ...
  if (config.useIPFSNetwork && config.helia) {
    try {
      ...
      if (!config.gatewayFallback) {
        throw new Error(
          `Could not download block ${cid} from IPFS network and gateway fallback is disabled`,
        );
      }
 ...
 * @param {"auto"|"blocks"|"car"} [options.strategy="auto"] - how blocks reach the
 *   backend; "auto" asks the backend what it can take (see chooseBackupStrategy)
...
export async function backupDatabase(orbitdb, databaseAddress, options = {}) {
  ...
  try {
    ...
    const strategy = chooseBackupStrategy(backend, config.strategy);
    if (strategy === "car") {
      logger.info(
        `🗜️  Backing up as a CAR: ${backend.name} cannot take blocks one by one`,
      );
      // Imported lazily: backup-car.js imports from this module, and a static import
      // here would close the cycle at load time.
      const { backupDatabaseCAR } = await import("./backup-car.js");
      return backupDatabaseCAR(orbitdb, databaseAddress, {
        ...options,
        backend,
      });
    }
