  ...
  const uploadSingleBlock = async ([hash, blockData]) => {
    try {
      ...
      const result = await retryWithBackoff(
        async () => await client.uploadFile(blockFile),
        {
          operationName: `Upload block ${hash.substring(0, 12)}...`,
...
/**
 * Download a block from Storacha/IPFS gateways with fallback and retry
 *
  ...
  for (const gateway of gateways) {
    try {
      const bytes = await retryWithBackoff(
        ...
        {
          operationName: `Download ${storachaCID.substring(0, 12)}... from ${gateway}`,
          maxRetries: 2,
        }
      );
      ...
      console.log(`   ⚠️ Failed from ${gateway} after retries: ${error.message}`);
    }
  }

  throw new Error(`Could not download block ${storachaCID} from any gateway after retries`);
}
