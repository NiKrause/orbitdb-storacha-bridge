...
/**
 * Every block in a CAR, by CID string.
 *
 * @param {Uint8Array} carBytes
 * @returns {Promise<Map<string, { bytes: Uint8Array }>>}
 */
export async function readBlocksFromCAR(carBytes) {
  const reader = await CarReader.fromBytes(carBytes);
  const blocks = new Map();
  for await (const block of reader.blocks()) {
    blocks.set(block.cid.toString(), { bytes: block.bytes });
  }
  return blocks;
}
...
export async function restoreFromCID(orbitdb, options = {}) {
  const { metadataCID, fetchBytes = fetchFromGateways, log = SILENT, ...rest } = options;
  ...
  // 2 · the CAR holds the blocks
  const blocks = await readBlocksFromCAR(await fetchBytes(carCID, { ...rest, log }));
