...
export function setLogLevel(level) {
  ...
  logger(`setLogLevel called with ${level} - use DEBUG environment variable instead`);
}
...
export function enableLogging(level = "info") {
  // No-op: libp2p logger uses DEBUG env var
  logger(`enableLogging called with ${level} - use DEBUG environment variable instead`);
}
