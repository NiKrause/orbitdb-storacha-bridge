...
async function demonstrateUCANRevocation() {
  ...
  const storachaKey =
    process.env.STORACHA_KEY || process.env.NEXT_PUBLIC_STORACHA_PRIVATE_KEY;
  const storachaProof =
    process.env.STORACHA_PROOF || process.env.NEXT_PUBLIC_STORACHA_DELEGATION;
  ...
  try {
    ...
    logger.info(
      { authorityDID: authorityPrincipal.did() },
      `   Authority identity: ${authorityPrincipal.did()}`,
    );
    ...
    logger.info(
      ...
      `   Recipient identity: ${recipientPrincipal.did()}`,
    );
    ...
    logger.info(
      { capabilities, recipientDID: recipientPrincipal.did() },
      `   Delegating capabilities: ${capabilities.join(", ")}`,
    );
    ...
    logger.info(
      { count: currentDelegations.length },
      `   Current delegations count: ${currentDelegations.length}`,
    );

    // Find our delegation in the list
    const ourDelegation = currentDelegations.find(
      (d) => d.cid.toString() === delegationCID.toString(),
    );
    if (ourDelegation) {
      logger.info(
        {
          delegationCID: ourDelegation.cid.toString(),
          audience: ourDelegation.audience.did(),
        },
        `   Found our delegation: ${ourDelegation.cid}`,
      );
      logger.info(`   Audience: ${ourDelegation.audience.did()}`);
      logger.info(
        { capabilities: ourDelegation.capabilities.map((c) => c.can) },
        `   Capabilities: ${ourDelegation.capabilities.map((c) => c.can).join(", ")}`,
      );
    }
    ...
    try {
      ...
      const revocationResult =
        await authorityClient.revokeDelegation(delegationCID);
      ...
      if (revocationResult.ok) {
        ...
        logger.warn(
          ...
          "   Revocation returned an error",
        );
      }
    } catch (revocationError) {
      logger.error({ error: revocationError.message }, "   Revocation failed");
      logger.info(
        "   This might be expected if the delegation was not found or already revoked",
      );
    }
    ...
    logger.info(
      { count: delegationsAfterRevocation.length },
      `   Delegations count after revocation: ${delegationsAfterRevocation.length}`,
    );

    const stillExists = delegationsAfterRevocation.find(
      (d) => d.cid.toString() === delegationCID.toString(),
    );
    if (stillExists) {
    ...
    try {
      ...
      logger.warn(
        ...
        "   Upload succeeded - revocation might not be immediate",
      );
    } catch (testError) {
    ...
    logger.info(
      "   Revocation may not be immediate due to caching/propagation",
    );
...
async function demonstrateCLIStyleRevocation() {
  ...
  logger.info("\n Command:");
  logger.info(
    "   w3 delegation ls                    # List all delegations with CIDs",
  );
  logger.info(
    "   w3 delegation revoke <delegation-cid> # Revoke specific delegation",
  );
...
if (import.meta.url === `file://${process.argv[1]}`) {
  demonstrateUCANRevocation()
    ...
    .catch((err) =>
      logger.error({ error: err.message, stack: err.stack }, "Demo error"),
    );
}
